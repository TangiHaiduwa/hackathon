...
const DoctorDrugAdmin = () => {
  ...
  const fetchPatients = async () => {
    ...
    try {
      ...
      const { data: presData, error: presError } = await supabase
        .from("prescriptions")
        .select(
          "id, patient_id, doctor_id, prescription_date, status_id (status_name)"
        )
  ...
  const fetchPatientData = async () => {
    ...
    try {
      ...
      const { data: presData, error: presError } = await supabase
        .from("prescriptions")
        .select(
          "id, patient_id, doctor_id, prescription_date, status_id (status_name), notes"
        )
  ...
  const recordAdministration = async () => {
    ...
    try {
      ...
      const { error } = await supabase.from("drug_administration").insert({
  ...
  const reportSideEffect = async () => {
    if (
      !newSideEffect.drug_administration_id ||
      !newSideEffect.side_effect_id
    ) {
  ...
  const updateComplianceData = async () => {
    try {
      ...
      const taken =
        recentAdministrations?.filter((a) => a.status === "administered")
          .length || 0;
      const compliance = scheduled > 0 ? (taken / scheduled) * 100 : 100;

      // Save compliance data
      await supabase.from("treatment_compliance").insert({
        patient_id: selectedPatient.id,
  ...
  const calculateOverallCompliance = () => {
    ...
    const totalCompliance = complianceData.reduce(
      (sum, record) => sum + record.compliance_percentage,
      0
    );
    ...
    <div className="card-medical">
      <div className="px-6 py-4 border-b">
        ...
        <p className="text-sm text-gray-600">
          Choose a patient to view drug administration records
        </p>
      </div>
      <div className="p-6">
            ...
            >
              <div>
                ...
                <span className="text-sm text-gray-600 ml-2">
                  {record.dosage_administered}
                </span>
                <div className="text-sm text-gray-500">
                  {new Date(record.actual_time).toLocaleDateString()} at{" "}
                  {new Date(record.actual_time).toLocaleTimeString()}
                </div>
              </div>
              <span
            ...
            <div key={record.id} className="border rounded-lg p-4">
              <div className="flex justify-between items-start mb-3">
                <div>
                  ...
                  <p className="text-sm text-gray-600">
                    {record.prescription_item_id.drug_id.dosage}
                  </p>
                  <p className="text-sm text-gray-500">
                    {record.prescription_item_id.dosage_instructions}
                  </p>
                </div>
                <span
                  className={`px-2 py-1 rounded text-xs ${
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                ...
                <div>
                  ...
                  <p>
                    {record.administered_by.first_name}{" "}
                    {record.administered_by.last_name}
                  </p>
                </div>
                <div>
              ...
              {record.patient_side_effects &&
                record.patient_side_effects.length > 0 && (
                  <div className="mt-3">
              ...
              <button
                onClick={() => {
                  setNewSideEffect((prev) => ({
                    ...prev,
                    drug_administration_id: record.id,
                  }));
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div>
            ...
            <select
              ...
              onChange={(e) =>
                setNewAdministration((prev) => ({
                  ...prev,
                  prescription_item_id: e.target.value,
                }))
              }
          ...
          <div>
            ...
            <input
              ...
              onChange={(e) =>
                setNewAdministration((prev) => ({
                  ...prev,
                  dosage_administered: e.target.value,
                }))
              }
              className="input-medical"
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div>
            ...
            <select
              ...
              onChange={(e) =>
                setNewSideEffect((prev) => ({
                  ...prev,
                  drug_administration_id: e.target.value,
                }))
              }
              className="input-medical"
            >
              ...
              {administrationRecords.map((record) => (
                <option key={record.id} value={record.id}>
                  {record.prescription_item_id.drug_id.drug_name} -{" "}
                  {new Date(record.actual_time).toLocaleDateString()}
                </option>
              ))}
            </select>
          </div>
          ...
          <div className="md:col-span-2">
            ...
            <textarea
              ...
              onChange={(e) =>
                setNewSideEffect((prev) => ({ ...prev, notes: e.target.value }))
              }
              className="input-medical"
