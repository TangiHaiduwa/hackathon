...
const DoctorMedicalRecords = () => {
  ...
  try {
    ...
    const { data: apptData, error: apptError } = await supabase
      .from('appointments')
      .select('id, patient_id, appointment_date, appointment_time, status_id (status_name)')
    ...
    const uniquePatients = apptData.map(apt => {
      ...
      return userData ? { ...userData, ...patientData, appointmentTime: apt.appointment_time, status: apt.status_id.status_name } : null;
  ...
  const searchPatients = async () => {
    try {
      const { data, error } = await supabase
        ...
        `)
        .or(`first_name.ilike.%${searchTerm}%,last_name.ilike.%${searchTerm}%,id.ilike.%${searchTerm}%`)
        .limit(20);
  ...
  const saveLabResult = async () => {
    ...
    try {
      ...
      setNewLabResult({ test_name: '', result: '', units: '', reference_range: '', notes: '' });
      await fetchPatientRecords(selectedPatient.id); // Refresh records
    } catch (error) {
          ...
          <div className="space-y-3 max-h-96 overflow-y-auto">
                  ...
                  <div>
                    ...
                    {patient.appointmentTime && (
                      <p className="text-sm text-gray-500">
                        Appointment: {patient.appointmentTime.substring(0, 5)} - {patient.status}
                      </p>
                    )}
                  </div>
            ...
            {patients.length === 0 && (
              <div className="text-center py-8 text-gray-500">
                ...
                <p>{searchTerm ? 'No patients found' : 'No appointments today'}</p>
              </div>
            )}
          </div>
        </div>
          ...
          <div>
            ...
            <dl className="space-y-2">
              <div>
                ...
                <dd className="text-sm text-gray-900">{patientRecords.first_name} {patientRecords.last_name}</dd>
              </div>
              <div>
                <dt className="text-sm font-medium text-gray-500">Date of Birth</dt>
                <dd className="text-sm text-gray-900">
                  {patientRecords.date_of_birth ? new Date(patientRecords.date_of_birth).toLocaleDateString() : 'N/A'}
                </dd>
              </div>
              <div>
                <dt className="text-sm font-medium text-gray-500">Contact</dt>
                <dd className="text-sm text-gray-900">{patientRecords.phone_number || 'N/A'}</dd>
                <dd className="text-sm text-gray-900">{patientRecords.email || 'N/A'}</dd>
              </div>
              <div>
                <dt className="text-sm font-medium text-gray-500">Address</dt>
                <dd className="text-sm text-gray-900">{patientRecords.address || 'N/A'}</dd>
              </div>
            </dl>
          </div>
          ...
          <div>
            ...
            <dl className="space-y-2">
              ...
              <div>
                ...
                <dd className="text-sm text-gray-900">{patientRecords.insurance_provider || 'None'}</dd>
                <dd className="text-sm text-gray-900">{patientRecords.insurance_number || ''}</dd>
              </div>
              <div>
                <dt className="text-sm font-medium text-gray-500">Emergency Contact</dt>
                <dd className="text-sm text-gray-900">{patientRecords.emergency_contact_name || 'None'}</dd>
                <dd className="text-sm text-gray-900">{patientRecords.emergency_contact_phone || ''}</dd>
              </div>
            </dl>
          </div>
        </div>
    ...
    <div className="card-medical">
      ...
      <div className="p-6">
        <div className="space-y-6">
                ...
                {patientRecords.diagnoses.map(diagnosis => (
                  <div key={diagnosis.id} className="border rounded-lg p-3">
                    <div className="flex justify-between items-start">
                      <div>
                        <span className="font-medium text-gray-900">{diagnosis.disease_id.disease_name}</span>
                        <span className="ml-2 text-sm text-gray-500">(ICD-10: {diagnosis.disease_id.icd_code})</span>
                        <p className="text-sm text-gray-600 mt-1">{diagnosis.notes}</p>
                      </div>
                      <div className="text-right text-sm text-gray-500">
                        <div>{new Date(diagnosis.diagnosis_date).toLocaleDateString()}</div>
                        <div>Dr. {diagnosis.doctor_id.first_name} {diagnosis.doctor_id.last_name}</div>
          ...
          {/* Medications */}
          <div>
            <h4 className="font-medium text-gray-900 mb-3">Medication History</h4>
            {patientRecords.prescriptions && patientRecords.prescriptions.length > 0 ? (
              <div className="space-y-3">
                    ...
                    <div className="flex justify-between items-start">
                      <div>
                        <span className="font-medium text-gray-900">
                          {prescription.prescription_items.map(item => item.drug_id.drug_name).join(', ')}
                        </span>
                      ...
                      <div className="text-right text-sm text-gray-500">
                        ...
                        <span className={`inline-block px-2 py-1 rounded text-xs ${
                          prescription.status_id.status_name === 'active' ? 'bg-green-100 text-green-800' :
          ...
          <div>
            ...
            {patientRecords.lab_results && patientRecords.lab_results.length > 0 ? (
              <div className="space-y-3">
                  ...
                  <div key={lab.id} className="border rounded-lg p-3">
                    <div className="flex justify-between items-start">
                      <div>
                        ...
                        <p className="text-lg font-bold text-blue-600">{lab.result} {lab.units}</p>
                        {lab.reference_range && (
                          <p className="text-sm text-gray-600">Ref: {lab.reference_range}</p>
                        )}
                        {lab.notes && <p className="text-sm text-gray-600 mt-1">{lab.notes}</p>}
                      </div>
                      <div className="text-right text-sm text-gray-500">
                        <div>{new Date(lab.performed_at).toLocaleDateString()}</div>
                        <div>By: {lab.ordered_by.first_name} {lab.ordered_by.last_name}</div>
                      </div>
                    </div>
                  </div>
                ))}
    ...
    <div className="card-medical">
      ...
      <div className="p-6">
        <div className="space-y-4">
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-2">Subjective (S)</label>
            <textarea
              rows="3"
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-2">Objective (O)</label>
            <textarea
              rows="3"
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-2">Assessment (A)</label>
            <textarea
              rows="3"
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-2">Plan (P)</label>
            <textarea
              rows="3"
              value={soapNote.plan}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            ...
            <input
              ...
              onChange={(e) => setNewLabResult(prev => ({ ...prev, test_name: e.target.value }))}
              className="input-medical"
          ...
          <div>
            ...
            <input
              ...
              onChange={(e) => setNewLabResult(prev => ({ ...prev, units: e.target.value }))}
              className="input-medical"
          ...
          <div>
            ...
            <input
              ...
              onChange={(e) => setNewLabResult(prev => ({ ...prev, reference_range: e.target.value }))}
              className="input-medical"
          ...
          <div className="md:col-span-2">
            ...
            <textarea
              ...
              onChange={(e) => setNewLabResult(prev => ({ ...prev, notes: e.target.value }))}
              className="input-medical"
  ...
  return (
    <DashboardLayout user={user} navigation={navigation}>
      ...
      <div className="mb-6">
        ...
        <p className="text-gray-600">Comprehensive patient history and clinical documentation</p>
      </div>
      ...
      {activeTab === 'soap' && (
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-1">
            <div className="card-medical">
              <div className="px-6 py-4 border-b">
                <h3 className="text-lg font-semibold text-gray-900">SOAP Guidelines</h3>
              </div>
              <div className="p-6 space-y-4 text-sm">
                <div>
                  <strong>Subjective:</strong> Patient's symptoms, concerns, medical history
                </div>
                <div>
                  <strong>Objective:</strong> Measurable findings, exam results, vital signs
                </div>
                <div>
                  <strong>Assessment:</strong> Diagnosis, clinical impression, problem list
                </div>
                <div>
                  <strong>Plan:</strong> Treatment, medications, follow-up, education
                </div>
              </div>
            </div>
          </div>
      ...
      {/* Navigation Back */}
      {(activeTab === 'records' || activeTab === 'soap' || activeTab === 'lab') && (
        <div className="mt-6">
          <button
