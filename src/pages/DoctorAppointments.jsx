...
const DoctorAppointments = () => {
  ...
  const fetchDoctorData = async () => {
    try {
      ...
      setUser({
        ...
        specialization: doctorProfile.medical_staff?.specialization_id?.specialization_name
      });
    } catch (error) {
  ...
  const fetchAppointments = async () => {
    try {
      ...
      const appointmentsWithDetails = apptData.map(apt => {
        const patient = patientMap.get(apt.patient_id) || { first_name: 'Unknown', last_name: '' };
        
        // Calculate urgency based on symptoms
        const urgencyScore = apt.appointment_symptoms?.reduce((score, symptom) => {
        ...
        return {
          ...
          symptoms: apt.appointment_symptoms?.map(s => s.symptom_id.symptom_name) || []
        };
      });
  ...
  const filterAppointments = () => {
    ...
    // Filter by status
    if (statusFilter !== 'all') {
      filtered = filtered.filter(apt => apt.status_id.status_code === statusFilter);
    }
  ...
  const renderCalendarView = () => {
    ...
    if (currentWeek.length > 0) weeks.push(currentWeek.concat(Array(7 - currentWeek.length).fill(null)));
        ...
        <div className="p-6">
          ...
          <div className="grid grid-cols-7 gap-2 mt-2">
            {weeks.flat().map((day, index) => {
              ...
              const dayAppts = filteredAppointments.filter(apt => apt.appointment_date === dateStr);
              const hasAppts = dayAppts.length > 0;
              return (
                <div
                  key={index}
                  className={`h-10 flex items-center justify-center text-sm rounded ${
                    hasAppts ? 'bg-blue-100 text-blue-800 font-medium cursor-pointer hover:bg-blue-200' : 'text-gray-700'
                  }`}
                  onClick={hasAppts ? () => setSelectedAppointment(dayAppts[0]) : undefined}  // Show first for simplicity; adjust if needed
                >
                  {day} {hasAppts && <span className="ml-1 text-xs">({dayAppts.length})</span>}
                </div>
              );
            })}
          </div>
            ...
            <div className="space-y-4">
              {filteredAppointments.map(apt => (
                ...
                >
                  <div className="flex justify-between items-start">
                    <div>
                      ...
                      <p className="text-sm text-gray-600">
                        {apt.appointment_date} at {apt.appointment_time.substring(0, 5)}
                      </p>
                      <span className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-medium mt-1 ${
                        apt.urgency === 'high' ? 'bg-red-100 text-red-800' :
                  ...
                  {apt.reason && (
                    <p className="text-sm text-gray-500 mt-2 truncate">{apt.reason}</p>
                  )}
                  {apt.symptoms.length > 0 && (
                    <p className="text-sm text-gray-500 mt-1">Symptoms: {apt.symptoms.join(', ')}</p>
                  )}
                </div>
              ))}
            </div>
          ) : (
            <p className="text-gray-500 text-center py-8">No appointments match the filters</p>
          )}
        </div>
      </div>
    );
          ...
          <div className="p-6 space-y-6">
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                <p className="font-medium">{selectedAppointment.patientAge || 'N/A'}</p>
              </div>
              <div>
              ...
              <div>
                ...
                <p className="font-medium">
                  {selectedAppointment.appointment_date} at {selectedAppointment.appointment_time.substring(0, 5)}
                </p>
              </div>
              <div>
                <p className="text-sm text-gray-500">Urgency</p>
                <span className={`inline-flex items-center px-2 py-1 rounded text-xs font-medium ${
                  selectedAppointment.urgency === 'high' ? 'bg-red-100 text-red-800' :
                  selectedAppointment.urgency === 'medium' ? 'bg-yellow-100 text-yellow-800' :
                  'bg-green-100 text-green-800'
                }`}>
                  {selectedAppointment.urgency.charAt(0).toUpperCase() + selectedAppointment.urgency.slice(1)}
                </span>
              </div>
            </div>
            <div>
              <p className="text-sm text-gray-500 mb-1">Reason</p>
              <p className="text-gray-900">{selectedAppointment.reason || 'No reason provided'}</p>
            </div>
            <div>
              ...
              {selectedAppointment.symptoms.length > 0 ? (
                <ul className="list-disc pl-5">
                  {selectedAppointment.symptoms.map((sym, i) => (
                    <li key={i} className="text-gray-900">{sym}</li>
                  ))}
                </ul>
              ) : (
          ...
          <div className="px-6 py-4 border-t flex justify-end space-x-3">
            ...
            <button
              onClick={() => {
                ...
                navigate(`/doctor-diagnosis?patientId=${selectedAppointment.patient_id}`);
              }}
  ...
  return (
    <DashboardLayout user={user} navigation={navigation}>
      <div className="mb-8">
        ...
        <p className="text-gray-600 mt-1">Manage and view your scheduled consultations</p>
      </div>
                  ...
                  {getTodaysAppointments().map(apt => (
                    ...
                    >
                      <div className="flex justify-between items-start">
                        <div>
                          <p className="font-medium text-gray-900">{apt.patientName}</p>
                          <p className="text-sm text-gray-600">{apt.appointment_time.substring(0, 5)}</p>
                          <span className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-medium mt-1 ${
                            apt.urgency === 'high' ? 'bg-red-100 text-red-800' :
                            apt.urgency === 'medium' ? 'bg-yellow-100 text-yellow-800' :
                            'bg-green-100 text-green-800'
            ...
            <div className="p-6 space-y-3">
              ...
              <div className="flex justify-between">
                ...
                <span className="font-medium text-green-600">
                  {appointments.filter(a => a.status_id.status_code === 'confirmed').length}
                </span>
              </div>
              <div className="flex justify-between">
                <span className="text-gray-600">Pending</span>
                <span className="font-medium text-yellow-600">
                  {appointments.filter(a => a.status_id.status_code === 'scheduled').length}
                </span>
              </div>
              <div className="flex justify-between">
