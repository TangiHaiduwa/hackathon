...
const DoctorDiagnosis = () => {
      ...
      if (!seenPatients.has(patientId)) {
        ...
        if (patient) {
          uniquePatients.push({
            ...
            preSelectedSymptoms: apt.appointment_symptoms?.map(s => s.symptom_id) || []
          });
  ...
  const performRuleBasedDiagnosis = () => {
    ...
    const malariaSymptoms = ['fever', 'chills', 'headache', 'nausea', 'vomiting', 'sweating', 'fatigue', 'abdominal pain', 'muscle pain'];
    const typhoidSymptoms = ['fever', 'headache', 'abdominal pain', 'constipation', 'diarrhea', 'rash', 'weakness', 'loss of appetite'];
    ...
    const malariaProbability = totalScore > 0 ? (malariaScore / totalScore) * 100 : 0;
    const typhoidProbability = totalScore > 0 ? (typhoidScore / totalScore) * 100 : 0;
    ...
    if (malariaProbability >= 60) {
      differentials.push({
        ...
        icd10: 'B54',
        confidence: 'High',
        supportingSymptoms: selectedSymptoms.filter(id => {
          const symptom = symptoms.find(s => s.id === id);
          return symptom && malariaSymptoms.some(ms => symptom.symptom_name.toLowerCase().includes(ms));
    ...
    if (typhoidProbability >= 60) {
      differentials.push({
        ...
        supportingSymptoms: selectedSymptoms.filter(id => {
          const symptom = symptoms.find(s => s.id === id);
          return symptom && typhoidSymptoms.some(ts => symptom.symptom_name.toLowerCase().includes(ts));
        }).map(id => symptoms.find(s => s.id === id).symptom_name)
      });
    }

    // Add other possible diagnoses based on symptoms
    if (selectedSymptoms.some(id => {
      const symptom = symptoms.find(s => s.id === id);
    ...
    setDiagnosisResults({
      ...
      differentialDiagnoses: differentials.sort((a, b) => b.probability - a.probability)
    });
  ...
  const saveDiagnosis = async () => {
    ...
    try {
      ...
      // Save vital signs
      if (clinicalFindings.temperature || clinicalFindings.bloodPressureSystolic) {
  ...
  const renderPatientSelection = () => (
    <div className="card-medical">
      <div className="px-6 py-4 border-b">
        ...
        <p className="text-sm text-gray-600">Choose from your upcoming appointments</p>
      </div>
      <div className="p-6">
            ...
            >
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-sm text-gray-500">Appointment: {new Date(patient.latestAppointment).toLocaleDateString()}</p>
                </div>
                <ChevronDownIcon className="h-5 w-5 text-gray-400" />
              </div>
              {patient.appointmentReason && (
                <p className="text-sm text-gray-600 mt-2">Reason: {patient.appointmentReason}</p>
              )}
              {patient.preSelectedSymptoms.length > 0 && (
                <div className="mt-2">
                  <span className="text-xs font-medium text-blue-600">Pre-reported symptoms:</span>
                  <div className="flex flex-wrap gap-1 mt-1">
                    {patient.preSelectedSymptoms.slice(0, 3).map(symptom => (
                      <span key={symptom.id} className="text-xs bg-blue-100 text-blue-800 px-2 py-1 rounded">
                        {symptom.symptom_name}
                      </span>
                    ))}
                    {patient.preSelectedSymptoms.length > 3 && (
                      <span className="text-xs text-gray-500">+{patient.preSelectedSymptoms.length - 3} more</span>
                    )}
                  </div>
                </div>
              )}
  ...
  const renderSymptomReview = () => (
    <div className="card-medical">
      <div className="px-6 py-4 border-b">
        ...
        <p className="text-sm text-gray-600">Review and add symptoms for {selectedPatient?.first_name}</p>
      </div>
      <div className="p-6">
        {/* Pre-selected symptoms */}
        {selectedPatient?.preSelectedSymptoms.length > 0 && (
          <div className="mb-6">
            <h4 className="font-medium text-gray-900 mb-3">Patient-Reported Symptoms</h4>
            <div className="flex flex-wrap gap-2">
              {selectedPatient.preSelectedSymptoms.map(symptom => (
                <span key={symptom.id} className="bg-green-100 text-green-800 px-3 py-1 rounded-full text-sm">
                  ✓ {symptom.symptom_name}
                </span>
              ))}
            </div>
        ...
        <div className="mb-4">
          <div className="flex justify-between items-center mb-3">
            <h4 className="font-medium text-gray-900">Additional Clinical Symptoms</h4>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3 max-h-96 overflow-y-auto">
            {symptoms.map(symptom => (
              <label key={symptom.id} className="flex items-start space-x-2 p-2 rounded-lg hover:bg-gray-50">
                <input
                  type="checkbox"
                ...
                />
                <div>
                  <span className="text-sm font-medium text-gray-700">{symptom.symptom_name}</span>
                  <div className="flex items-center space-x-2 mt-1">
                    <span className={`text-xs px-2 py-1 rounded ${
                      symptom.category_id?.weight === 4 ? 'bg-red-100 text-red-800' :
                      symptom.category_id?.weight === 3 ? 'bg-orange-100 text-orange-800' :
                      symptom.category_id?.weight === 2 ? 'bg-yellow-100 text-yellow-800' :
                      'bg-blue-100 text-blue-800'
                    }`}>
                      {symptom.category_id?.category_name} ({symptom.category_id?.weight})
                    </span>
                  </div>
                </div>
              </label>
  ...
  const renderClinicalFindings = () => (
    <div className="card-medical">
      <div className="px-6 py-4 border-b">
        <h3 className="text-lg font-semibold text-gray-900">Clinical Findings</h3>
        <p className="text-sm text-gray-600">Record vital signs and physical examination</p>
      </div>
      <div className="p-6">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) => setClinicalFindings(prev => ({ ...prev, temperature: e.target.value }))}
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <input
                  ...
                  onChange={(e) => setClinicalFindings(prev => ({ ...prev, bloodPressureSystolic: e.target.value }))}
                  className="input-medical"
                  placeholder="120"
                />
              </div>
              <div>
                ...
                <input
                  ...
                  onChange={(e) => setClinicalFindings(prev => ({ ...prev, bloodPressureDiastolic: e.target.value }))}
                  className="input-medical"
                  placeholder="80"
                />
              </div>
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) => setClinicalFindings(prev => ({ ...prev, heartRate: e.target.value }))}
                className="input-medical"
                placeholder="72"
              />
            </div>
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) => setClinicalFindings(prev => ({ ...prev, respiratoryRate: e.target.value }))}
                className="input-medical"
                placeholder="16"
              />
            </div>
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) => setClinicalFindings(prev => ({ ...prev, oxygenSaturation: e.target.value }))}
                className="input-medical"
                placeholder="98.0"
              />
            </div>
          ...
          <div className="md:col-span-2">
            <h4 className="font-medium text-gray-900 mb-3">Physical Examination</h4>
            <textarea
              rows="6"
              value={clinicalFindings.physicalExam}
              onChange={(e) => setClinicalFindings(prev => ({ ...prev, physicalExam: e.target.value }))}
              className="input-medical"
              placeholder="General appearance, systems review, notable findings..."
            />
            
            <h4 className="font-medium text-gray-900 mt-4 mb-3">Clinical Notes</h4>
            <textarea
              rows="3"
              value={clinicalFindings.notes}
              onChange={(e) => setClinicalFindings(prev => ({ ...prev, notes: e.target.value }))}
              className="input-medical"
  ...
  const renderDiagnosisResults = () => (
    <div className="space-y-6">
      ...
      <div className="card-medical">
        <div className="px-6 py-4 border-b">
          <h3 className="text-lg font-semibold text-gray-900">Rule-Based Diagnosis Results</h3>
        </div>
        <div className="p-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
            <div className="text-center p-4 bg-blue-50 rounded-lg">
              <div className="text-2xl font-bold text-blue-600">{diagnosisResults.malariaProbability}%</div>
              <div className="text-sm text-blue-800">Malaria Probability</div>
            </div>
            <div className="text-center p-4 bg-orange-50 rounded-lg">
              <div className="text-2xl font-bold text-orange-600">{diagnosisResults.typhoidProbability}%</div>
          ...
          {diagnosisResults.requiresChestXray && (
            <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4 mb-4">
              <div className="flex items-center">
                ...
                <span className="font-medium text-yellow-800">Chest X-ray Recommended</span>
              </div>
              <p className="text-sm text-yellow-700 mt-1">
                Very strong signs detected. Recommend chest X-ray in addition to standard treatment.
              </p>
            </div>
          )}
        </div>
              ...
              >
                <div className="flex items-center justify-between">
                  ...
                  <div className="text-right">
                    <div className="text-lg font-bold text-blue-600">{diagnosis.probability}%</div>
                ...
                {diagnosis.supportingSymptoms && (
                  <div className="mt-2">
                    <span className="text-xs font-medium text-gray-500">Supporting Symptoms:</span>
                    <div className="flex flex-wrap gap-1 mt-1">
          ...
          {finalDiagnosis && (
            <div className="mt-6 p-4 bg-green-50 border border-green-200 rounded-lg">
              <div className="flex items-center">
                ...
                <span className="font-medium text-green-800">Final Diagnosis Selected</span>
              </div>
              <p className="text-sm text-green-700 mt-1">
                {finalDiagnosis.disease} (ICD-10: {finalDiagnosis.icd10}) - {finalDiagnosis.confidence} confidence
              </p>
            </div>
          )}
  ...
  const renderTreatmentPlan = () => (
    <div className="card-medical">
      <div className="px-6 py-4 border-b">
        ...
        <p className="text-sm text-gray-600">Create treatment plan for {finalDiagnosis?.disease}</p>
      </div>
      <div className="p-6">
        {/* Treatment recommendations based on diagnosis */}
        {finalDiagnosis && (
          <div className="mb-6">
            <h4 className="font-medium text-gray-900 mb-3">Recommended Treatment for {finalDiagnosis.disease}</h4>
            <div className="bg-blue-50 p-4 rounded-lg">
              {finalDiagnosis.disease === 'Malaria' && (
                <div>
                  <p className="text-sm text-blue-800">
                    <strong>First-line treatment:</strong> Artemether-lumefantrine (Coartem) or Artesunate-based therapy
                  </p>
              ...
              {finalDiagnosis.disease === 'Typhoid Fever' && (
                <div>
                  <p className="text-sm text-blue-800">
                    <strong>First-line treatment:</strong> Azithromycin or Ceftriaxone for severe cases
                  </p>
                  <p className="text-sm text-blue-700 mt-1">
        ...
        <div className="space-y-6">
            ...
            <div className="space-y-3">
              {treatmentPlan.medications.map((med, index) => (
                <div key={index} className="flex items-center space-x-3 p-3 bg-gray-50 rounded-lg">
                  <span className="flex-1">{med.name} - {med.dosage}</span>
                  <button
          ...
          <div>
            ...
            <textarea
              ...
              onChange={(e) => setTreatmentPlan(prev => ({ ...prev, followUp: e.target.value }))}
              className="input-medical"
          ...
          <div>
            ...
            <textarea
              ...
              onChange={(e) => setTreatmentPlan(prev => ({ ...prev, instructions: e.target.value }))}
              className="input-medical"
  ...
  const renderPatientInfoSidebar = () => {
    ...
    return (
      <div className="space-y-6">
        ...
        <div className="card-medical">
          ...
          <div className="p-6">
            <div className="text-center mb-4">
              ...
              <h4 className="font-medium text-gray-900">{selectedPatient.first_name} {selectedPatient.last_name}</h4>
              <p className="text-sm text-gray-600">
            ...
            <div className="space-y-3">
              <div>
                ...
                <p className="text-sm">{new Date(selectedPatient.latestAppointment).toLocaleDateString()}</p>
              </div>
        ...
        <div className="card-medical">
          <div className="px-6 py-4 border-b">
            <h3 className="text-lg font-semibold text-gray-900">Selected Symptoms</h3>
          </div>
          <div className="p-6">
            ...
            {selectedSymptoms.length === 0 && (
              <p className="text-sm text-gray-500 text-center">No symptoms selected</p>
            )}
          </div>
        </div>
        ...
        <div className="card-medical">
          <div className="px-6 py-4 border-b">
            <h3 className="text-lg font-semibold text-gray-900">Diagnosis Progress</h3>
          </div>
          <div className="p-6">
            <div className="space-y-4">
              {['patient', 'symptoms', 'clinical', 'diagnosis', 'treatment'].map((step, index) => (
                <div key={step} className="flex items-center">
                  <div className={`w-6 h-6 rounded-full flex items-center justify-center text-xs ${
                    activeTab === step ? 'bg-blue-600 text-white' :
                    index < ['patient', 'symptoms', 'clinical', 'diagnosis', 'treatment'].indexOf(activeTab) 
                  ...
                  <span className={`ml-3 text-sm ${
                    activeTab === step ? 'font-medium text-blue-600' : 'text-gray-600'
                  }`}>
  ...
  return (
    <DashboardLayout user={user} navigation={navigation}>
      ...
      <div className="mb-6">
        ...
        <p className="text-gray-600">Comprehensive diagnostic tool with rule-based expert system</p>
      </div>
